...
def test_input(config, sess, model, images, idx):
    ...
    # y_rnn_mask (2000,)
    trials = {}
    trials['x'] = np.zeros((1,11))
    trials['y_rnn'] = np.zeros((2000, 3))
    trials['y_rnn_mask'] = np.zeros((2000,))
    feed_dict = {model.x: trials['x'],
                 model.y_rnn: trials['y_rnn'],
                 model.y_rnn_mask: trials['y_rnn_mask']}
    # print('x', np.array(trials['x']).shape)
    # print('y_rnn', np.array(trials['y_rnn']).shape)
    # print('y_rnn_mask', np.array(trials['y_rnn_mask']).shape)
